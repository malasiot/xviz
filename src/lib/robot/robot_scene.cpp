...
RobotScenePtr RobotScene::fromURDF(const URDFRobot &r) {
     return parseRobotURDF(r) ;
}

static NodePtr createLinkGeometry(const URDFGeometry *urdf_geom, MaterialPtr mat, Vector3f &scale) {
...
RobotScenePtr RobotScene::parseRobotURDF(const URDFRobot &rb)
{
    ...
    for( const auto &lp: rb.links_ ) {
        ...
        if ( link.visual_geom_ ) {
            ...
            URDFGeometry *geom = link.visual_geom_.get() ;
